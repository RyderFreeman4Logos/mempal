...
| MCP-010 | `mempal_ingest` | `supported` | MCP, daemon queue | installed-live smoke | `full_smoke.py` probes `mcp_create`, `mcp_update`; MCP hard-timeout wrapper; operation receipt recovery | REST fallback may keep cleanup possible but does not make `mcp_create` pass. | Current live CRUD failure is a regression if it returns. |
...
| SAFE-002 | Cleanup-safe ID authority | `supported` | CLI, MCP, smoke runner | unit, installed-live smoke | `test_created_ids_from_accepts_only_cleanup_safe_fields`; `full_smoke.py` cleanup probes `cli_delete_batch`, `mcp_delete_batch` | No automatic cleanup if create failed before returning cleanup-authorized IDs. | Search/read result IDs are not cleanup authority. |
