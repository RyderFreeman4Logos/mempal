...
- **Daemon/MCP/CLI coexistence**: daemons open current schemas beside live MCP writers, avoid bootstrap queue reclamation, retry transient SQLite stalls, tolerate bounded writer-lease contention, and retain schema rejection/repair, query-only reads, durable receipts, and bounded drains; deterministic fixtures cover timing and lease lifecycles (#843, #849, #850, #853, #856, #858, #859). Hermes SessionEnd ingest now uses indexed SQL session filters instead of full-table post-filtering (#864).
