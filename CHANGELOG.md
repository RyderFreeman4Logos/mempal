...
- **Daemon/MCP/CLI ingest liveness**: make daemon status queue diagnostics read-only and short-bounded; preserve followable deterministic MCP operation receipts when daemon delivery is uncertain; and route `ingest --stdin --wait --json` through queue admission before a write-capable database open, with structured no-write receipts (#843).
