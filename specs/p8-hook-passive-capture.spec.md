...
Scenario: mempal hook 对 >10MB payload envelope-wrap 且 UTF-8 边界安全
  Test:
    Filter: test_hook_envelopes_oversized_payload
    Level: integration
    Targets: crates/mempal-cli/src/hook.rs
  Given stdin 输入 11MB JSON payload，且在字节偏移 4093-4097 区间跨越一个中文字符（三字节 UTF-8 序列）作为 char-boundary 切断测试
  When 执行 `mempal hook hook_post_tool`
  Then 命令仍退出码 "0"
  And 命令**不 panic**（UTF-8 char boundary 切断不触发 `&str` slicing panic）
  ...
  And `payload_preview` 字节长度 <= 4096
  And `payload_preview` 是合法 UTF-8 字符串（`std::str::from_utf8` 成功；无 dangling 多字节字符）
...
Scenario: truncated envelope 的 payload_preview 经 privacy scrub 后不含原始 secret
  Test:
    Filter: test_truncated_envelope_preview_is_scrubbed
    Level: integration
    Targets: crates/mempal-cli/src/daemon.rs, crates/mempal-ingest/src/privacy.rs
  Given `[privacy] enabled = true` 且默认 `openai_key` pattern 启用
  And stdin 输入 11MB payload，其前 4KB 含 `sk-abcdef1234567890abcdef1234567890abcd` 字面值
  When 执行 `mempal hook hook_post_tool` 然后启动 `mempal daemon --foreground` 并等 5s
  Then `drawers` 表新增 1 行，`wing == "hooks-raw"`，`room == "truncated"`
  And 该 drawer `content` 含 `[REDACTED:openai_key]`
  And 该 drawer `content` **不含**原始 `sk-abcdef1234567890abcdef1234567890abcd` 字面值
