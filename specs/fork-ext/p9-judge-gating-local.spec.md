...
- fork-ext `fork_ext_version` `2 → 3` bump（新增 `gating_audit` 表）
...
- `crates/mempal-core/src/db/schema.rs`（fork_ext_version `2 → 3` migration + `gating_audit` DDL）
...
Scenario: fork-ext 迁移 v2 → v3 创建 gating_audit 表
  Test:
    Filter: test_fork_ext_migration_v2_to_v3_creates_gating_audit
    Level: integration
    Targets: crates/mempal-core/src/db/schema.rs
  Given palace.db `fork_ext_version == "2"`（embed-qwen3 reindex_progress 已就位）
  When 启动 mempal
  Then `fork_ext_version == "3"`
