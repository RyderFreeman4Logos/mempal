...
async fn test_search_and_ingest_during_partial_reindex() {
    ...
    // Queued receipt must return while reindex holds write locks; completion waits until resume.
    let receipt = server
        ...
        .await
        .expect("ingest receipt during reindex")
        .0;
    assert_eq!(
        receipt.state,
    ...
    let ingest = server
        ...
        .await
        .expect("ingest completion after reindex");
    assert_eq!(
        ingest.state,
