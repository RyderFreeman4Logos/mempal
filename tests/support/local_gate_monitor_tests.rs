#[cfg(test)]
const _: fn() = descendant_monitor_delayed_start_is_joined_after_release;

#[cfg(test)]
pub(super) fn descendant_monitor_delayed_start_is_joined_after_release() {
    ...
    let monitor_result =
        DescendantMonitor::spawn_delayed_for_test(root.identity, start_release_receiver);
    start_release_sender
        .send(())
        .expect("release delayed monitor worker");
    let mut monitor = monitor_result.expect("a delayed monitor worker must not fail construction");
