...
impl AcquiredMcpIngestWriterLease {
    fn into_guard(mut self) -> McpIngestWriterLeaseGuard {
        let lease = self
            .lease
            .take()
            .expect("acquired ingest writer lease must be present");
        McpIngestWriterLeaseGuard::new(std::mem::take(&mut self.db_path), lease)
    }
}

impl Drop for AcquiredMcpIngestWriterLease {
    fn drop(&mut self) {
        if let Some(lease) = self.lease.take()
            && let Ok(db) = Database::open_lease_control(&self.db_path)
        {
            let _ = db.runtime_writer_lease_release(&lease);
        }
    }
}
...
impl MempalMcpServer {
    ...
    #[cfg(test)]
    fn with_ingest_writer_lease_acquired_hook_for_test(
        mut self,
        hook: IngestWriterLeaseAcquiredHook,
    ) -> Self {
        self.ingest_writer_lease_acquired_hook = Some(hook);
        self
    }
        ...
        loop {
            ...
            if let Some(observer) = &self.daemon_write_observer {
                match &claim_result {
                    ...
                    Ok(None) => {}
                    Err(error) => observer
                        .record_queue_error("failed to claim next MCP ingest operation", error),
                }
            }
            match claim_result {
                Ok(Some(claim)) => {
    ...
    async fn process_ingest_claim_inline(
            ...
            && mcp_ingest_failure_is_retryable_writer_lease_lost(detail)
        {
            ...
            Self::release_claim_with_lock_retry(queue, claim, self.daemon_write_observer.as_ref())
                .await
        ...
        let heartbeat = tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = interval.tick() => {
                        let result = queue
                            .refresh_heartbeat(claim_id.clone(), heartbeat_worker_id.clone())
                            .await;
                        if let Some(observer) = &write_observer {
                            observer.observe_maintenance_queue_result(
    ...
    async fn refresh_ingest_claim_heartbeat_once(
        ...
        if let Some(observer) = write_observer {
            observer.observe_maintenance_queue_result(
                format!("failed to refresh async ingest heartbeat before completion {claim_id}"),
                &result,
            );
        }
        match result {
    ...
    async fn acquire_ingest_writer_lease(
        ...
        let lease = tokio::task::spawn_blocking(move || {
            ...
            #[cfg(test)]
            if let (Some(hook), Some(lease)) = (acquired_hook.as_ref(), lease.as_ref()) {
                hook(lease);
            }
            Ok::<_, anyhow::Error>(lease.map(|lease| AcquiredMcpIngestWriterLease {
                db_path: admitted_path,
                lease: Some(lease),
            }))
        })
        .await
        .context("MCP ingest writer lease task failed")??;
        Ok(lease.map(AcquiredMcpIngestWriterLease::into_guard))
    }
    ...
    async fn ensure_async_pool_for_wait_ingest(&self) -> std::result::Result<(), ErrorData> {
        match self.async_db().await {
            ...
            Err(error) => {
                ...
                if anyhow_chain_has_holder_budget_exceeded(&error) {
                    diagnostic.failure_kind = "holder_budget_exceeded".to_string();
                    diagnostic.hint = status_db_failure_hint(&diagnostic.failure_kind).to_string();
                }
                if diagnostic.failure_kind == "holder_budget_exceeded" {
    ...
    async fn release_scoped_claim_after_timeout(
        ...
        Self::release_claim_with_lock_retry_deadline(queue, claim, retry_deadline, write_observer)
            .await
            .map_err(|error| {
                ErrorData::internal_error(
    ...
    async fn release_claim_with_lock_retry_deadline(
        ...
        loop {
            ...
            if let Some(observer) = write_observer {
                observer.observe_maintenance_queue_result(
                    format!("failed to release MCP ingest claim {}", claim.id),
                    &result,
                );
            }
            match result {
...
fn status_db_failure_kind(error: &(dyn std::error::Error + 'static)) -> &'static str {
    ...
    while let Some(error) = current {
        ...
        if matches!(
            error.downcast_ref::<crate::core::db::DbError>(),
            Some(crate::core::db::DbError::RuntimeWriterLeaseLost { .. })
        ) {
...
fn anyhow_chain_has_holder_budget_exceeded(error: &anyhow::Error) -> bool {
    error.chain().any(|cause| {
        matches!(
...
fn status_database_diagnostic(
    ...
    if failure_kind == "locked_or_busy" {
        ...
        if status_error_summary(error).contains("current_mcp_server") {
            hint.push_str(" Holder class: current_mcp_server.");
        }
    }
    DatabaseDiagnosticDto {
...
fn budget_exceeded_profile_admission(error: &anyhow::Error) -> Option<serde_json::Value> {
    let admission =
        error.chain().find_map(
            |cause| match cause.downcast_ref::<crate::core::db::DbError>() {
                Some(crate::core::db::DbError::Admission(
                ...
                _ => cause
                    .downcast_ref::<crate::core::db_admission::DbAdmissionError>()
                    .filter(|cause| {
                        matches!(
                            cause,
                            crate::core::db_admission::DbAdmissionError::BudgetExceeded { .. }
...
async fn requeue_ingest_claim_after_transient_write_lock(
    ...
    if let Some(observer) = write_observer {
        observer.observe_maintenance_queue_result(
            format!("failed to requeue MCP ingest claim {}", claim.id),
            &result,
        );
    }
...
async fn complete_ingest_operation_with_lock_retry_deadline(
    ...
    loop {
        ...
        let result = queue
            ...
            .await;
        if let Some(observer) = write_observer {
            observer.observe_semantic_queue_result(
                format!("failed to complete MCP ingest claim {}", claim.id),
                &result,
            );
        }
        match result {
            ...
            Err(error) if error.is_sqlite_lock() && started.elapsed() < retry_deadline => {
                let remaining = retry_deadline.saturating_sub(started.elapsed());
                tracing::warn!(?error, operation_id = %claim.id, "retrying async ingest result persistence after SQLite lock");
...
mod tests {
    ...
    async fn test_mcp_content_mutation_rejects_takeover_after_preflight() {
        ...
        assert!(
            error.message.contains("writer_lease_lost"),
            "error={error:?}"
        );
...
"#,
    ...
    include!("server/tests/ingest_malformed_receipt_tests.rs");
