...
#[path = "db_writer_lease_liveness.rs"]
mod db_writer_lease_liveness;
use db_writer_lease_liveness::{
    runtime_boot_id, runtime_writer_daemon_is_live_holder, runtime_writer_lease_holder_is_live,
...
impl Database {
    ...
    pub(super) fn record_gating_audit_in_current_transaction(
        &self,
        candidate_hash: &str,
        decision: &GatingDecision,
        project_id: Option<&str>,
    ...
    pub fn update_drawer_after_merge_and_record_novelty_audit(
        ...
        self.update_drawer_after_merge_and_record_novelty_audit_fenced(
            None,
    ...
    pub fn upsert_drawer_and_replace_vector(
        ...
        self.upsert_drawer_and_replace_vector_fenced(None, drawer, vector)
    }
    ...
    pub fn pin_drawer(&self, drawer_id: &str, pin_order: Option<i64>) -> Result<bool, DbError> {
        self.pin_drawer_fenced(None, drawer_id, pin_order)
    }

    pub fn pin_drawer_fenced(
        &self,
        lease: Option<&RuntimeWriterLease>,
        drawer_id: &str,
        pin_order: Option<i64>,
    ) -> Result<bool, DbError> {
        self.with_runtime_writer_lease_transaction(lease, "pin drawer", || {
    ...
    pub fn pin_drawers_fenced(
        &self,
        lease: Option<&RuntimeWriterLease>,
        drawer_ids: &[String],
    ) -> Result<(), DbError> {
        self.with_runtime_writer_lease_transaction(lease, "pin drawers", || {
    ...
    pub fn replace_active_source_drawers(
        ...
        self.replace_active_source_drawers_fenced(
            None,
            source_file,
            wing,
            room,
    ...
    pub fn replace_active_source_drawers_fenced(
        &self,
        lease: Option<&RuntimeWriterLease>,
        source_file: &str,
        wing: &str,
        ...
        self.with_runtime_writer_lease_transaction(lease, "replace source drawers", || {
            self.replace_active_source_drawers_tx(source_file, wing, room, project_id, source_root)
        })
    }

    fn replace_active_source_drawers_tx(
        &self,
        source_file: &str,
        wing: &str,
        room: Option<&str>,
        ...
        self.delete_source_drawer_rows_tx(rows, project_id, source_root)
    }
    ...
    pub fn replace_active_source_drawers_across_rooms(
        ...
        self.replace_active_source_drawers_across_rooms_fenced(
            None,
            source_file,
            wing,
            project_id,
    ...
    pub fn replace_active_source_drawers_across_rooms_fenced(
        &self,
        lease: Option<&RuntimeWriterLease>,
        source_file: &str,
        wing: &str,
        ...
        self.with_runtime_writer_lease_transaction(lease, "replace source drawers", || {
            self.replace_active_source_drawers_across_rooms_tx(
                source_file,
                wing,
                project_id,
                source_root,
    ...
    fn replace_active_source_drawers_across_rooms_tx(
        &self,
        source_file: &str,
        wing: &str,
        project_id: Option<&str>,
    ...
    fn delete_source_drawer_rows_tx(
        &self,
    ...
    pub fn apply_ingest_boost_batch(
        ...
        self.apply_ingest_boost_batch_fenced(
            None,
            IngestBoostBatch {
                drawer_ids,
                now_ms,
    ...
    pub fn apply_ingest_boost_batch_fenced(
        &self,
        lease: Option<&RuntimeWriterLease>,
        batch: IngestBoostBatch<'_>,
    ) -> Result<(), DbError> {
        if batch.drawer_ids.is_empty() {
            return Ok(());
        }
        let sql = r#"
            UPDATE drawers SET
        ...
        "#;
        let mut stmt = self.conn.prepare_cached(sql)?;
        self.with_runtime_writer_lease_transaction(lease, "apply ingest boost", || {
            for id in batch.drawer_ids {
                stmt.execute(rusqlite::params![
                    batch.boost_per_access,
                    batch.now_ms,
                    batch.decay_rate,
                    batch.floor,
                    batch.boost_cap,
    ...
    fn runtime_writer_lease_acquire_with_cleanup_policy(
        ...
        let persisted_metadata_json = runtime_writer_metadata_with_process_identity(metadata_json);
        self.with_immediate_tx(|| {
            ...
            if policy == RuntimeWriterLeaseAcquirePolicy::RebindDeadDaemon {
                let previous_holders = {
                    let mut statement = self.conn.prepare(
                        "SELECT owner, pid, boot_id, session_id, generation, mode, metadata_json \
                ...
                {
                    let previous_should_retain = u32::try_from(previous_pid).ok().is_some_and(|pid| {
                        runtime_writer_lease_holder_should_retain(
                            &previous_owner,
                            pid,
                            previous_boot_id.as_deref(),
                            &previous_mode,
                            previous_metadata_json.as_deref(),
                        )
                    });
                    if !previous_should_retain {
                        self.conn.execute(
        ...
        if acquired {
            ...
            Ok(Some(RuntimeWriterLease {
                ...
                mode: mode.to_string(),
                metadata_json: Some(persisted_metadata_json),
                remaining_secs,
            }))
        } else {
            Ok(None)
    ...
    fn runtime_writer_lease_cleanup_expired_tx(
        ...
        let expired = {
            let mut stmt = self.conn.prepare(
                "SELECT name, owner, pid, boot_id, session_id, generation, mode, metadata_json \
                 FROM runtime_writer_leases \
        ...
        let mut removed = 0;
        for (name, owner, pid, boot_id, session_id, generation, mode, metadata_json) in expired {
            let retain_holder = preserve_live_holders
